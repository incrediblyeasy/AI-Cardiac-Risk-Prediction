# Paper 2 — Implementation Notes

Full code for **Paper 2 (CausalEchoNet)**, built from the plan in
`PROJECT_STATUS_AND_ROADMAP.md` §3. It follows Paper 1's conventions
(rationale-heavy docstrings, nested-dataclass + JSON configs,
...
keeps `tests/` and `configs/` at the top level, so this package does too — that
...
## Tests
`tests/test_paper2_*.py` — all green. Runtime-gated entry points are themselves
tested (they must raise their specific error), so a gate can't silently rot into
a no-op.

## What is still NOT here (inherently needs you)
- **Headline results** — the GPU-scale Paper 1 runs + the frozen-encoder export
  that unlocks Paper 2 training. No fabricated numbers.
- **Credentialed / downloaded data** — INCART/PTB-XL downloads for the Paper 1
  external-validation runs Paper 2 sits downstream of.
